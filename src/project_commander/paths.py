...
- Oh-My-Pi: same scheme but typically rooted at the user's home, so the
  `/home/<user>` prefix is stripped, leaving `-code-foo`.
...
def omp_key(path: Path, home: Path) -> str:
    ...
    OMP prefixes are relative to the user's home: `/home/<user>/code/foo`
    becomes `-code-foo` (the home prefix is dropped).
    """
